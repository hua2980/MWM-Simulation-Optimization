...
while ((track_x(end) - platform_x)^2 + (track_y(end) - platform_y)^2 > ... 
    ...
    %Check if not out of bounds, reset location & speed if so
        if (track_x(end)^2 + track_y(end)^2 > (pool_diameter/2)^2)
            % repeat action selection if at the wall of maze
            if (length(track_x) > 2.5 && track_x(end-2)^2 + track_y(end-2)^2 > (0.99*pool_diameter/2)^2)
% (length(track_x) > 2.5 && track_x(end-2)^2 + track_y(end-2)^2 > (0.999*pool_diameter/2)^2)
                track_x = track_x(1:end-1);
                track_y = track_y(1:end-1);
                vel_x = vel_x(1:end-1);
                vel_y = vel_y(1:end-1);
                el_tr = el_tr0;
                not_done = true;
            else
%         vel_x0 = track_x(end)-track_x(end-1);
%         vel_y0 = track_y(end)-track_y(end-1);
%         ratio = (track_x(end)^2 + track_y(end)^2)/((pool_diameter/2)^2);
%         track_x(end) = track_x(end)/sqrt(ratio);
%         track_y(end) = track_y(end)/sqrt(ratio);
%         ratio_2 = (track_x(end-1)^2 + track_y(end-1)^2)/((pool_diameter/2)^2);
%         acc_x = -1 * track_x(end)/(pool_diameter/2);
%         acc_y = -1 * track_y(end)/(pool_diameter/2);
%         vel_x(end)= -1*(sqrt(vel_x0^2 + vel_y0^2-(acc_x*vel_x0+acc_y*vel_y0)^2))* acc_y
%         vel_y(end)= (sqrt(vel_x0^2 + vel_y0^2-(acc_x*vel_x0+acc_y*vel_y0)^2))* acc_x      
        ratio = (track_x(end)^2 + track_y(end)^2)/((pool_diameter/2)^2);
        track_x(end) = track_x(end)/sqrt(ratio);
        track_y(end) = track_y(end)/sqrt(ratio);
    ...
    platform_radius^2)
       ...
       elseif (track_x(end)^2+track_y(end)^2 > (0.98*pool_diameter/2)^2)        %%%%%%%%%%%%%why 0.99 maybe try to set the wall reward constant at 0? or -10?
